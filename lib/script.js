...
import { displaySummaryTable } from './chartModule.js'; 
import { exportAllCollectionsToCSV } from './exportModule.js'; 
...
export const COLLECTION_IDS = {
    ...
    activityCategories: '685d7dd2001b35eaf00c' 
};
...
export function switchView(viewId) {
    ...
    const filterHolidayTypeWrapper = document.getElementById('filterHolidayTypeWrapper'); 
    ...
    document.getElementById('dataContainer').classList.add('hidden'); // Default hidden for all views
    ...
    if (copyButtonsContainer) copyButtonsContainer.classList.add('hidden'); 
    if (copyGraphButtonWrapper) copyGraphButtonWrapper.classList.add('hidden'); 
    ...
    switch (viewId) {
        case 'table':
            ...
            document.getElementById('dataContainer').classList.remove('hidden'); // Show data container for table view
            if (filterTeamMemberWrapper) filterTeamMemberWrapper.classList.remove('hidden');
            if (filterActivityWrapper) filterActivityWrapper.classList.remove('hidden'); 
            if (copyButtonsContainer) copyButtonsContainer.classList.remove('hidden'); 
            displayFilteredTimeEntries();
            break;
        case 'charts':
            document.getElementById('filterControls').classList.remove('hidden');
            document.getElementById('dataContainer').classList.remove('hidden'); // <-- CHANGED: Show data container for charts (for the summary table)
            document.getElementById('utilizationChartContainer').classList.remove('hidden');
            if (copyButtonsContainer) copyButtonsContainer.classList.remove('hidden'); 
            if (copyGraphButtonWrapper) copyGraphButtonWrapper.classList.remove('hidden'); 
        ...
        case 'holidays':
            ...
            if (filterTeamMemberWrapper) filterTeamMemberWrapper.classList.remove('hidden');
            if (filterHolidayTypeWrapper) filterHolidayTypeWrapper.classList.remove('hidden'); 
            if (copyButtonsContainer) copyButtonsContainer.classList.remove('hidden'); 
            displayHolidays();
            break;
        case 'manage':
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (addTeamMemberForm) {
        addTeamMemberForm.addEventListener('submit', async (e) => {
            ...
            if (name && !isNaN(dailyHours) && dailyHours >= 0) {
                await addTeamMember(name, dailyHours, userId); 
            } else {
    ...
    if (addActivityCategoryForm) {
        addActivityCategoryForm.addEventListener('submit', async (e) => {
            ...
            const userId = getOrCreateGuestUserId(); 
            if (categoryName) {
                await addActivityCategory(categoryName, userId); 
            } else {
    ...
    document.getElementById('exportDataBtn')?.addEventListener('click', exportAllCollectionsToCSV);
